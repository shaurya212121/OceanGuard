    ...
    <header className="h-16 bg-navy-900 border-b border-line flex items-center justify-between px-6 sticky top-0 z-10">
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-2 text-sm text-text-muted font-mono font-semibold">
          <div className="w-1.5 h-1.5 bg-safe" /> [ SAT-LINK ACTIVE ]
        </div>
        <div className="text-text-muted font-mono tracking-wider">{time.toISOString().substring(11, 19)} UTC</div>
        <button className="relative p-2 text-text-muted hover:text-text transition-colors border border-transparent hover:border-line">
          <Bell size={20} />
          <span className="absolute top-1 right-1 w-2 h-2 bg-danger" />
        </button>
      </div>
    </header>
  );
